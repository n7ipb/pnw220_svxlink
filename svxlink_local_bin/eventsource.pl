...
my $state_pty = "/tmp/state_repeater";
...
sub parse($) {
    ...
    if (($parsed{"event"} eq "Voter:sql_state") | ($parsed{"event"} eq "MultiTx:state")) {
        foreach $item (@$rx_info) {
          ...
          if ( ($parsed{"event"} eq "MultiTx:state")) {  
              $lvl = $item->{transmit} + 0;

            $parsed{"tx"}{$rx}{"sql"} = $sql;
            $parsed{"tx"}{$rx}{"lvl"} = $lvl;
        } else {
